...
public class ICarLeaseRepositoryImpl implements ICarLeaseRepository {
	...
	{
		try {
			ps = con.prepareStatement("insert into vehicle(vehicleid,make,model,year,dailyrate,status,passengercapacity,enginecapacity) values(?,?,?,?,?,?,?,?)");
			//ps.setInt(1, car.getCarID()); set to auto_increment in mysql
			ps.setString(1,car.getMake());
			ps.setString(2,car.getModel());
			ps.setInt(3,car.getYear());
			ps.setFloat(4, car.getDailyRate());
			ps.setInt (5, car.getStatus());
	...
	public List<Car> listRentedCars() {
		...
		try {
			...
			{
				Car car = new Car();
				car.setCarID(rs.getInt("vehicleId"));
			    car.setMake(rs.getString("make"));
			    car.setModel(rs.getString("model"));
			    car.setYear(rs.getInt("year"));
			    car.setDailyRate(rs.getFloat("dailyRate"));
	...
	@Override
	public Lease LeaseStatus(int leaseID,int customerID, int carID, LocalDate startDate, LocalDate endDate) {
